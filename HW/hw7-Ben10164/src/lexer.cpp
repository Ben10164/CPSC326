...
Token Lexer::next_token()
{
    ...
    if (curr_char == '\'')
    {
        ...
        if ((isalpha(peek()) || ispunct(peek()) || (peek() == ' ')) && peek() != '\\')
        {
    ...
    {
        if (curr_char == '0' && peek() != '.' && peek() != ' ' && peek() != ';' && peek() != '\n' && isdigit(peek())) // leading zero detection
        {
        ...
        while (peek() != EOF)
        {
            ...
            if (curr_char == ' ' || curr_char == '\n' || isalpha(peek()) || (!isdigit(peek()) && peek() != '.')) // check for alphabetic char
            {
                if (period_present)
                {
    ...
    while (peek() != EOF || peek() != ' ' || peek() != '\n')
    {
        ...
        if (peek() == '(' || peek() == ')' || peek() == '=' || peek() == '{' || peek() == '}' || peek() == '[' || peek() == ']' || peek() == '+' || peek() == '-' || peek() == '*' || peek() == '/' || peek() == '\'' || peek() == '"' || peek() == '+' || peek() == '.' || peek() == ',' || peek() == ';' || peek() == '\n'|| peek() == '>'|| peek() == '<' || peek() == '!')
        {
            break;
        }
        curr_char = read();
