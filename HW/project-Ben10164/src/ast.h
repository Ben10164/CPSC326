...
// Had to reorder this in order to compile it because VarDef now contains an expression
//----------------------------------------------------------------------
...
public:
  DataType data_type;
  Token var_name;
  std::optional<Expr> defaultExpr = std::nullopt;
...
public:
  ...
  std::vector<Expr> keyword_args; // this is a parallel list corresponding with args
  bool keyword_values = false;
  void accept(Visitor& v) { v.visit(*this); }  
