...
void SemanticChecker::visit(AssignStmt& s)
{
    ...
    s.expr.accept(*this);
    if(((curr_type.type_name != lval_type.type_name) && curr_type.type_name != "void") || (curr_type.is_array != lval_type.is_array))
        error("Type mismatch: " + curr_type.type_name);
...
void SemanticChecker::visit(CallExpr& e)
{
    ...
    if (fun_name == "print"){
        ...
        // count number of variables with default values
        int num_def = 0;
        for (auto p : f.params){
            if(p.defaultExpr.has_value()){
                num_def ++;
        ...
        if (e.args.size() + num_def < f.params.size())
            error("Different number of arguments for function '" + f.fun_name.lexeme() + "'.");
        // check each arg type:
        for (int i = 0; i < e.args.size(); ++i) {
            if(e.keyword_values){
                int index = -1;
                DataType param_type = f.params[i].data_type;
                string name = e.args[i].first_token().lexeme();
                for(int i =0; i < e.keyword_args.size(); i ++){
                    std::string uh = e.args.at(i).first_token().lexeme();
                    if (e.args.at(i).first_token().lexeme() == name){
                        index = i;
                        break;
                    }
                }
