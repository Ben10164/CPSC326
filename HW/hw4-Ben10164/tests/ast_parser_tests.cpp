...
TEST(BasicASTParserTests, EmptyForStatement) {
  ...
  ASSERT_EQ("i", s.assign_stmt.expr.first->first_token().lexeme());
  ASSERT_EQ("+", s.assign_stmt.expr.op->lexeme());
  ASSERT_EQ(0, s.stmts.size());
}
...
TEST(BasicASTParserTests, ReturnStatement) {
  ...
  Program p = ASTParser(Lexer(in)).parse();
  ReturnStmt& s = (ReturnStmt&)*p.fun_defs[0].stmts[0];
  ASSERT_EQ("1", s.expr.first_token().lexeme());
}

// Five expression tests

TEST(BasicASTParserTests, Expr1) {
  stringstream in(build_string({
        "void main() {", 
        "  x = (300 + 2222) + 100", 
        "}"
  ...
  ASSERT_EQ("+", e.op.value().lexeme());
  SimpleRValue& v = (SimpleRValue&)*((SimpleTerm&)*(*e.rest).first).rvalue;
  ASSERT_EQ("100", v.value.lexeme());
}

TEST(BasicASTParserTests, Expr2) {
  stringstream in(build_string({
        "void main() {", 
        "  x = (300 + 2222) + 0", 
        "}"
...
TEST(BasicASTParserTests, Expr3) {
  stringstream in(build_string({
        "void main() {", 
        "  x = 300 + (2222 + 100)", 
        "}"
...
TEST(BasicASTParserTests, Expr4) {
  stringstream in(build_string({
        "void main() {", 
        "  x = (300) - (2222 + 100)", 
        "}"
...
TEST(BasicASTParserTests, Expr5) {
  stringstream in(build_string({
        "void main() {", 
        "  x = (300) -0- (2222 + 100)", 
        "}"
...
TEST(BasicASTParserTests, multi1) {
  stringstream in(build_string({
    "int main(){",
    "    int i = 0",
    "    i = i +2",
...
TEST(BasicASTParserTests, multi2) {
  stringstream in(build_string({
    "int add(int num1, int num2){",
    "    return num1+num2",
    "}",
...
TEST(BasicASTParserTests, multi3) {
  stringstream in(build_string({
    "int add(int num1, int num2){",
    "    return num1+num2",
    "}",
